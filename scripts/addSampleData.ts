...
export async function addSampleFoodItems() {
  try {
    console.log('Starting to add sample food items...');
    ...
    for (const item of sampleFoodItems) {
      ...
      const docRef = await addDoc(collection(db, 'foodItems'), itemWithTimestamps);
      console.log(`Added: ${item.name} (ID: ${docRef.id})`);
    }
    
    console.log('✅ Successfully added all sample food items!');
    return true;
  } catch (error) {
